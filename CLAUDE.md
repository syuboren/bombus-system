...
進行 CSS/版面變更時（例如加入固定標頭、導航列或覆蓋層），**務必考慮對相鄰元素的間距影響**。為內容容器加入適當的 `padding-top`/`margin-top`，以防止內容被固定定位元素遮住。套用修復前，先列出此變更將影響的所有視覺元素，並在同一次編輯中加入補償 CSS。

### 整合測試

執行整合測試時，**務必先驗證 `.env` 憑證是否與實際資料庫狀態一致**。首先檢查硬編碼值、不一致的 API 端點，以及 `.env` 與資料庫之間的憑證同步問題。修復測試前先做環境審計，不要盲目修改程式碼。

### 工具與指令

- 本專案使用 **Claude Code**（不是 Cursor）。自訂指令放在 `.claude/commands/` 目錄下的 markdown 檔案中，不是 `.agent/skills/`
- 在建議安裝或更新指令之前，**先驗證該指令在目前的工具/平台上確實存在**。不要在未確認其對使用者環境有效的情況下建議指令或方法
