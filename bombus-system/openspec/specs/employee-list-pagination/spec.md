# Employee List Pagination — 員工清單伺服器端分頁

## Purpose

Define the server-side pagination, search, and sort contract for the `GET /api/employee/list` endpoint, supporting tenants with hundreds to thousands of employees while preserving backward compatibility with legacy callers (card view, matrix view) that consume the array response. Also defines the supporting compound database index.
