# Department Template Import — 部門範本快速導入

## Purpose

定義 Bombus 部門範本快速導入能力，協助租戶在組織建立初期快速產生符合產業／規模特性的部門結構。涵蓋平台層級的部門範本字典（`department_templates` + `industry_dept_assignments`）、租戶端依產業＋規模查詢範本、組織架構頁的「自行新增／範本庫導入／批次匯入」三入口，以及 validate-then-execute 的匯入流程（含衝突偵測、overwrite/merge 模式、原子交易、審計日誌）。本能力同時定義 `departments.responsibilities` 重新命名為 `departments.value` 的資料表遷移，以及部門代碼產生器（D-15）的整合掛勾。
