...
- 新增 `server/src/services/platform-publisher/` 目錄：
  ...
  - `base.js`：抽象介面 `{ publish(jobData), update(platformJobId, jobData), close(platformJobId), reopen(platformJobId) }`（四方法固定；**不含 delete**，刪除職缺統一以 close 收斂，詳見下方 delete 行為變更）
  - `104.adapter.js`：包裝既有 `services/104/job.service.js`（postJob / updateJob / patchJobStatus；`deleteJob` 既有端點不再呼叫）
...
- **BREAKING：Delete 行為變更**：本地刪除職缺時，**改呼叫 `adapter.close()` 而非過去的 `job104Service.deleteJob`**。理由：(1) adapter 介面只有 4 方法、不擴張 scope；(2) 外部平台保留「已關閉」歷史比「無紀錄」對稽核友善；(3) 本地列仍經 `ON DELETE CASCADE` 清除，使用者感知一致。若未來需要強制刪除外部資料，再另立一次變更討論
